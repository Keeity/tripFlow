'use strict';

const { queryInterface, Sequelize } = require("sequelize");
//const User = require("../../models/User");
const bcrypt = require('bcryptjs');

module.exports = {
up: async (queryInterface, Sequelize) => {
await queryInterface.bulkinsert('users',[
//escreve em formato de objeto os alunos que quer inserir
{
    name: 'Keeity Braga Collodel',
    gender: 'Feminino',
    birthDate: '1986-09-17',
    cpf: '121.134.789-00',
    phone: '(48)92994321',
    email: 'keeity@example.com',
    password: bcrypt.hashSync('ytieek', 8),
    cep: '88063-300',
    address: 'Avenida Campeche, Florianópolis, SC',
    addressNumber: 123,
    addressComplement: 'Apto 12',
    role: 'admin',
    createdAt: '2024-05-05',
    updatedAt: '2024-05-05'
  },
  {     name: 'João Silva',
        gender: 'Masculino',
        birthDate: '1956-09-17',
        cpf: '123.456.789-00',
        phone: '(11) 98765-4321',
        email: 'joao.silva@example.com',
        password: bcrypt.hashSync('oaojjj', 8),
        cep: '89253-000',
        address: 'José Emmendoerfer, Nova Brasília, Jaraguá do Sul, SC',
        addressNumber: 321,
        role: 'user',
        createdAt: '2024-05-05',
        updatedAt: '2024-05-05'
      },

      {
        name: 'Maria Santos',
        gender: 'Feminino',
        birthDate: new Date('1980-01-01'),
        cpf: '987.654.321-00',
        phone: '(11) 12345-6789',
        email: 'maria.santos@example.com',
        password: bcrypt.hashSync('airamm', 8),
        cep: '23450-789',
        address: 'Rua Santa Cruz da Conceição, Parque Belém, São Paulo, SP',
        addressNumber: 456,
        addressComplement: 'Apto 34',
        role: 'user',
        createdAt: '2024-05-05',
        updatedAt: '2024-05-05'
      },
      {
        name: 'Carlos Oliveira',
        gender: 'Masculino',
        birthDate: new Date('1990-02-02'),
        cpf: '876.543.210-00',
        phone: '(11) 23456-7890',
        email: 'carlos.oliveira@example.com',
        password: bcrypt.hashSync('solrac', 8),
        cep: '334560-890',
        address: 'Rua Sete, Santa Cruz, Rio de Janeiro, RJ',
        addressNumber: 789,
        addressComplement: 'Apto 56',
        role: 'premiumUser',
        createdAt: '2024-05-05',
        updatedAt: '2024-05-05'
      },
      {
        name: 'Ana Souza',
        gender: 'Feminino',
        birthDate: '2000-03-03',
        cpf: '765.432.109-00',
        phone: '(11) 34567-8901',
        email: 'ana.souza@example.com',
        password: bcrypt.hashSync('anaaaa', 8),
        cep: '45678-901',
        address: 'Avenida das Nações Unidas, 11633, Brooklin Paulista, São Paulo, SP',
        addressNumber: 12,
        addressComplement: 'Apto 78',
        role: 'user',
        createdAt: '2024-05-05',
        updatedAt: '2024-05-05'
      }

])

},


down: async (queryInterface, Sequelize) => {
await queryInterface.bulkDelete('users',{
where: { email: [
        'keeity@example.com', 'joao.silva@example.com','maria.santos@example.com','carlos.oliveira@example.com', 'ana.souza@example.com'
    ]}
})
}
}